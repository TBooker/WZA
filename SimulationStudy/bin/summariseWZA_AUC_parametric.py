...
## Calculate the parametric p-value for each of the tests
		temp["Z_empirical_p_uncor"] = 1 - (temp["kendall_Z"].rank(method = "min")/ temp.shape[0])

		temp["empZ_parametric_p_uncor"] = 1 - (temp["empR_Z"].rank(method = "min")/ temp.shape[0])

		if SNP_intervals:
			pass
		else:
			temp["TC_parametric_p_uncor"] = 1 - ((-1*np.log10(temp["top_candidate_p_TC2"])).rank(method = "first")/ temp.shape[0])

		temp["SNP_parametric_p_uncor"] =  (temp["kendall_rank"].rank(method = "min")/ temp.shape[0])

		if BayPass:
			temp["Z_parametric_p_bayP"] = 1 - (temp["bayPass_Z"].rank(method = "min")/ temp.shape[0])
			temp["TC_parametric_p_bayP"] = 1 - ((-1*np.log10(temp["BP_top_candidate_p_TC2"])).rank(method = "first")/ temp.shape[0])
			temp["SNP_parametric_p_bayP"] =  (temp["BF_rank"].rank(method = "min")/ temp.shape[0])

		if RDA:
			temp["Z_parametric_p_RDA"] = 1 - (temp["RDA_Z"].rank(method = "min")/ temp.shape[0])
			temp["TC_parametric_p_RDA"] = 1 - ((-1*np.log10(temp["RDA_top_candidate_p_TC2"])).rank(method = "first")/ temp.shape[0])
			temp["SNP_parametric_p_RDA"] =  (temp["RDA_rank"].rank(method = "min")/ temp.shape[0])
...
## Get the total Covariance(phen, env) across all genes - remember that I have to exclude adjacent genes (i.e. marked with LA = -99)
		...
		for n in range(1,1001):
			if n%100==0:
				print("\t",n)
## Uncorrected data
			WZA_slice_uncor = temp[temp["Z_parametric_p_uncor"] <= (n)/temp.shape[0]]
			WZA_slice_empR = temp[temp["empZ_parametric_p_uncor"] <= (n)/temp.shape[0]]
			...
			if SNP_intervals:
				...
				TC_slice_uncor = temp[temp["TC_parametric_p_uncor"] <= (n)/temp.shape[0]]

			SNP_slice_uncor = temp[temp["SNP_parametric_p_uncor"] <= (n)/temp.shape[0]]

			if BayPass:
				WZA_slice_bayPass = temp[temp["Z_parametric_p_bayP"] <= (n)/temp.shape[0]]
				TC_slice_bayPass = temp[temp["TC_parametric_p_bayP"] <= (n)/temp.shape[0]]
				SNP_slice_bayPass = temp[temp["SNP_parametric_p_bayP"] <= (n)/temp.shape[0]]

			if RDA:
				WZA_slice_RDA = temp[temp["Z_parametric_p_RDA"] <= (n)/temp.shape[0]]
				TC_slice_RDA = temp[temp["TC_parametric_p_RDA"] <= (n)/temp.shape[0]]
				SNP_slice_RDA = temp[temp["SNP_parametric_p_RDA"] <= (n)/temp.shape[0]]
